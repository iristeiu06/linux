				 ...
				 int *val2, long m)
{
	...
	case IIO_CHAN_INFO_FREQUENCY: {
		...
		if (chan->type == IIO_ALTVOLTAGE) {
			...
			freq = (u64)phy->adi_adrv904x_device.initExtract.tx
				.txChannelCfg[chan->channel].rfBandwidth_kHz * 1000;
		} else {
			freq = (u64)phy->adi_adrv904x_device.initExtract.rx
				.rxChannelCfg[chan->channel].rfBandwidth_kHz * 1000;
		}
				  ...
				  int val2, long mask)
{
	...
	case IIO_CHAN_INFO_HARDWAREGAIN:
		if (chan->output)
			ret = adrv904x_write_tx_gain(phy, chan, val, val2);
		else
			ret = adrv904x_write_rx_gain(phy, chan, val, val2);
		break;
	case IIO_CHAN_INFO_FREQUENCY: {
		...
		loConfig.loName = ADI_ADRV904X_LO0 + chan->channel;
		ret = adrv904x_api_call(phy, adi_adrv904x_LoFrequencySet, &loConfig);
		break;
	}
	case IIO_CHAN_INFO_SAMP_FREQ:
		break;
				      ...
				      struct jesd204_link *lnk)
{
	...
	case DEFRAMER1_LINK_TX:
	{
		...
		const adi_adrv904x_DeframerSettingsExtract_t *deframer =
			&phy->kororDevice->initExtract.jesdSetting.deframerSetting[source_id - 1];
...
static int adrv904x_jesd204_post_running_stage(struct jesd204_dev *jdev,
					       enum jesd204_state_op_reason reason)
{
	adi_adrv904x_TrackingCalibrationMask_e trackingCal = (adi_adrv904x_TrackingCalibrationMask_e)0;
	adi_adrv904x_InitCalibrations_e currentInitCalMask = (adi_adrv904x_InitCalibrations_e)0;
	struct adrv904x_jesd204_priv *priv = jesd204_dev_priv(jdev);
	...
	const u32 ALL_CHANNELS_MASK = GENMASK(7, 0);
	const u32 NUM_TRACKING_CALS = 7;
	u32 init_chans = 0;
	u8 i, j;
	int ret;

	const u32 trackingCalMask = ADI_ADRV904X_TC_RX_ADC_MASK     |
